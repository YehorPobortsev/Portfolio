...
GROUP BY Driver_name,nationality,day_of_birth, dr.driverid
...
FROM f1.races AS r
	JOIN f1.circuits AS c
	ON r.circuitId = c.circuitId
GROUP BY country, location, c.name, c.circuitId
ORDER BY country
...
WHERE points != 0
GROUP BY constructorRef,year,c.constructorId,c.name
ORDER BY year desc
...
FROM CTE_pitstop
GROUP BY Year, Constructor, track_name,circuitId,constructorId
ORDER BY year DESC
